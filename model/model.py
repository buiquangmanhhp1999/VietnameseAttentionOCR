...
def conv(x, channels, kernel_size, stride, scope, normalizer_fn=slim.batch_norm, activation_fn=relu):
    return slim.conv2d(x, channels, kernel_size, stride, scope=scope, normalizer_fn=normalizer_fn,
                       activation_fn=activation_fn)


def inception_padding_model(images, labels, wemb_size, seq_len, num_classes, lstm_size, is_training=True,
                            dropout_keep_prob=0.5, weight_decay=0.00004, final_endpoint='Mixed_6h', name='InceptionV4',
                            reuse=None):
    """
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse) as scope:
        ...
        with slim.arg_scope(inception_v4_arg_scope()):
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                net, end_points = inception_v4_base(images, final_endpoint=final_endpoint,
                                                    scope=scope)  # Mixed_6h Mixed_7d
                ...
                with tf.compat.v1.variable_scope("attention_lstm"):
                    with tf.compat.v1.variable_scope('word_embedding'):
                        W_wemb = tf.compat.v1.get_variable('W_wemb', [num_classes, wemb_size],
                                                           initializer=xavier_initializer())

                    with tf.compat.v1.variable_scope("feature_map_attention"):
                        with tf.compat.v1.variable_scope("init_mean"):
                            W_init_c = tf.compat.v1.get_variable('W_init_c', [channel, lstm_size],
                                                                 initializer=xavier_initializer(),
                                                                 regularizer=regularizer)
                            W_init_h = tf.compat.v1.get_variable('W_init_h', [channel, lstm_size],
                                                                 initializer=xavier_initializer(),
                                                                 regularizer=regularizer)
                        with tf.compat.v1.variable_scope("attention_x"):
                            W = tf.compat.v1.get_variable('W', [channel, channel], initializer=xavier_initializer(),
                                                          regularizer=regularizer)
                        with tf.compat.v1.variable_scope("attention_h"):
                            W_h = tf.compat.v1.get_variable('W_h', [lstm_size, channel],
                                                            initializer=xavier_initializer(),
                                                            regularizer=regularizer)  # tf.truncated_normal_initializer(stddev=0.1)
                        with tf.compat.v1.variable_scope("att"):
                            W_att = tf.compat.v1.get_variable('W_att', [channel, 1], initializer=xavier_initializer(),
                                                              regularizer=regularizer)
                            b_att = tf.compat.v1.get_variable('b_att', [1], initializer=xavier_initializer())

                    with tf.compat.v1.variable_scope("softmax"):
                        softmax_w = tf.compat.v1.get_variable('softmax_w', [wemb_size, num_classes],
                                                              initializer=xavier_initializer(), regularizer=regularizer)
                        softmax_b = tf.compat.v1.get_variable('softmax_b', [num_classes],
                                                              initializer=xavier_initializer())  # tf.constant_initializer(biivector)

                    with tf.compat.v1.variable_scope("attention_to_embedding"):
                        W_attention_wemb = tf.compat.v1.get_variable('W_attention_wemb', [channel, wemb_size],
                                                                     initializer=xavier_initializer(),
                                                                     regularizer=regularizer)
                        W_hidden_wemd = tf.compat.v1.get_variable('W_hidden_wemd', [lstm_size, wemb_size],
                                                                  initializer=xavier_initializer(),
                                                                  regularizer=regularizer)

                    with tf.compat.v1.variable_scope('lstm_cell'):  # orthogonal_initializer  xavier_initializer()
                        lstm_W = tf.compat.v1.get_variable('lstm_W', [wemb_size, lstm_size * 4],
                                                           initializer=tf.initializers.orthogonal(),
                                                           regularizer=regularizer)
                        lstm_U = tf.compat.v1.get_variable('lstm_U', [lstm_size, lstm_size * 4],
                                                           initializer=tf.initializers.orthogonal(),
                                                           regularizer=regularizer)
                        lstm_Z = tf.compat.v1.get_variable('lstm_Z', [channel, lstm_size * 4],
                                                           initializer=tf.initializers.orthogonal(),
                                                           regularizer=regularizer)
                ...
                def _LSTMCell(wemb_prev, h_prev, attention_feature, c_prev, forget_bias=1., keep_prob=1.):
                    ...
                    """
                    pack = tf.add_n(
                        [tf.matmul(wemb_prev, lstm_W), tf.matmul(h_prev, lstm_U), tf.matmul(attention_feature, lstm_Z)])
                ...
                def attention_lstm(i, cnn_feature, wemb_prev, hidden_state, cell_state, output_array, attention_array):
                    ...
                    hidden_state, cell_state = _LSTMCell(wemb_prev, hidden_state, attention_feature, cell_state,
                ...
                _, _, _, _, _, output_array, attention_array = tf.while_loop(cond=lambda i, *_: i < seq_len,
                                                                             body=attention_lstm, \
                                                                             loop_vars=(
                                                                             tf.constant(0, tf.int32), cnn_feature,
                                                                             init_wemb, init_h, init_c, output_array,
                                                                             attention_array))  # , shape_invariants=
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse) as scope:
        ...
        with slim.arg_scope(inception_v4_arg_scope()):
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                ...
                def attention_lstm(i, cnn_feature, bboxes, wemb_prev, hidden_state, cell_state, output_array,
                                   attention_array):
                    """
                    Loop body for AttentionOCR.
                    Args:
                    ...
                    def map_fn_for_attention(image, bbox, attention_h):
                        ...
                        offset_height, offset_width, target_height, target_width = tf.cast(bbox[0] * height, tf.int32), \
                                                                                   tf.cast(bbox[1] * width,
                                                                                           tf.int32), tf.cast(
                            bbox[2] * height, tf.int32), tf.cast(bbox[3] * width, tf.int32)
                        image = tf.compat.v1.image.crop_to_bounding_box(image, offset_height, offset_width,
                        ...
                        paddings = [[offset_height, height - target_height - offset_height],
                                    [offset_width, width - offset_width - target_width]]
                        alpha = tf.pad(alpha, paddings, 'CONSTANT')

                        return attention_feature, alpha

                    # rnn hidden state transform  (batch, lstm_size) -> attention feature h (batch, channel)
                    attention_h = tf.matmul(hidden_state,
                                            W_h)  # (batch, channel)   attention feature in channel dimension
                ...
                _, _, _, _, _, _, output_array, attention_array = tf.while_loop(cond=lambda i, *_: i < seq_len,
                                                                                body=attention_lstm, \
                                                                                loop_vars=(
                                                                                tf.constant(0, tf.int32), cnn_feature,
                                                                                bboxes, init_y, init_h, init_c,
                                                                                output_array,
                                                                                attention_array))  # , shape_invariants=
...
if __name__ == '__main__':
    ...
    outputs, attentions = inception_model(input_placeholder, label_batch, bbox_batch, wemb_size=256, seq_len=32,
                                          num_classes=10000, \
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=tf.compat.v1.GPUOptions(allow_growth=True),
        ...
        for i in range(1000):
            before_op = time.time()
            results = sess.run([preds], feed_dict={input_placeholder: np.random.random_sample((1, size, size, 3)),
