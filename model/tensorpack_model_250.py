...
class AttentionOCR(ModelDesc):
    ...
    def inputs(self):
        return [tf.TensorSpec([None, cfg.height_img, cfg.image_size, 3], tf.float32, 'image'),
    ...
    @staticmethod
    def get_inferene_tensor_names():
        inputs, outputs = [], []
        if cfg.model_name == 'ocr':
            inputs, outputs = ['image', 'label', 'is_training', 'dropout_keep_prob'], ['sequence_preds',
                                                                                       'sequence_probs']
        elif cfg.model_name == 'ocr_with_normalized_bbox':
            inputs, outputs = ['image', 'label', 'normalized_bbox', 'is_training', 'dropout_keep_prob'], [
                'sequence_preds', 'sequence_probs']
    ...
    def build_graph(self, image, label, mask, normalized_bbox, is_training, dropout_keep_prob):
        if cfg.model_name == 'ocr':
            outputs, attentions = inception_padding_model(image, label, \
                                                          wemb_size=cfg.wemb_size, seq_len=cfg.seq_len + 1,
                                                          num_classes=cfg.num_classes, \
                                                          lstm_size=cfg.lstm_size, is_training=is_training,
                                                          dropout_keep_prob=dropout_keep_prob, \
                                                          weight_decay=cfg.weight_decay, name=cfg.name_scope,
                                                          reuse=None)
